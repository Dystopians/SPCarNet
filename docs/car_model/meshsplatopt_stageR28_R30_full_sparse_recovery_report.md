...
R44 turns the R43 diagnosis into a mechanism: sparse COLMAP depth loss is useful early, but should decay after the sparse geometry has anchored the recovery. R44.01 repairs the parking long-horizon sparse-recovery failure, improving PSNR by `+0.920386`, SSIM by `+0.037679`, and LPIPS by `-0.035538` versus R43.01b. R44.02 improves every courtyard metric versus R43.02b, including normal angle by `-2.429404` degrees. The direct clean-baseline visual comparison is saved under `outputs/carnet/meshsplatopt/baseline_vs_method_qualitative`, and the corrected best-clean-long comparison is saved under `outputs/carnet/meshsplatopt/best_clean_long_vs_method_long`.

## Current Interpretation

The strongest validated parking render result is the clean current-branch 22000-iteration baseline, not R44.01. The earlier R16.03 clean 7000-iteration comparison is a weak historical reference and must not be used as the main clean-baseline claim. In the corrected best-long comparison, clean 22k reaches PSNR `18.479990`, SSIM `0.634623`, LPIPS `0.346913`, AbsRel `0.082177`, and Depth MAE `1.868398`, while R44.01 reaches PSNR `17.169540`, SSIM `0.548714`, LPIPS `0.441888`, AbsRel `0.187067`, and Depth MAE `2.919396`. R44.01 only wins on normal proxy (`42.218251` vs `45.108437` degrees) and topology size (`782982` vs `8548242` triangles). R31 adds an important early-stop result and two cross-scene positives. R32 adds a concrete algorithmic improvement on top of long-horizon sparse-geometry-guided recovery: confidence-aware COLMAP sparse correspondence sampling. R33-R36 narrow the claim into a stronger paper-safe version: trusted sampling is a tunable confidence knob, not a universal constant fraction. R38-R44 then convert that tuning into a stronger lambda, budget, and decay curve: `lambda=0.001-0.002` is now the validated low-lambda regime, R44.01 is a parking topology/normal Pareto row rather than a render-best row, R44.02 is the courtyard long-horizon render/normal row, R40.02 remains the all-metric courtyard depth row, and R41.01 is the large bonsai render Pareto row.
