...
proxy-to-render mismatch, not by a missing carrier-count hyperparameter. The
first render-trust replay at scale `0.5` also fails train-val promotion
(`garden` balanced delta `-0.000004113`), so the next implemented pivot is a
render-visible region carrier proposal generator rather than another scale
scan. That pivot now has full9 evidence: the default render-visible region
prior gate accepts `4 / 9`, but `bonsai` exposes a train-val/test mismatch and
drives effective fallback metrics negative. A train-val-only robust promotion
layer fixes this by requiring no mean LPIPS regression plus tail-CVaR and
stratified-group floors; it keeps only `garden/kitchen` and yields full9
