# SPCarNet 当前方法完整技术报告 v18

日期：2026-06-23 14:53 -0700

用途：mentor 汇报、PPT 制作、当前方法交底、后续论文路线讨论。

当前主讲 endpoint：`ours_26000_phasej_guarded_adaptedge_ela`

当前项目状态：`NOT COMPLETE`。Phase-J 是当前最可靠、最适合放在主结果页的强结果；v48/v52 是表示级内化方向的最新证据，但收益仍明显小于 Phase-J，不能把 v52 包装成已经替代 Phase-J 的最终论文方法。v52 source-config W&B rerun 已经启动，正在 GPU 4 上运行，尚未完成全 9 场景复现。

---

## 0. 一页结论

SPCarNet 的核心目标不是从零替代 MeshSplatting，而是在一个已经训练好的 MeshSplatting checkpoint 上增加一个训练证据驱动的压缩、修复和风险控制阶段。
...
| 与 MeshSplatting paper table | Phase-J mean `26.4828 / 0.7837 / 0.2243`，paper table mean `24.78 / 0.728 / 0.310`；只作为 sanity check |
...
- 诚实边界是：它目前选择已经 materialized 的 v48/v51 结果；source-config GPU rerun 已经启动但尚未完成。
...
| 14 | Risks | render-time adapter、定性 subtle、v52 source rerun 未完成 |
| 15 | Next Step | 完成 W&B-logged v52 GPU rerun；推进高容量 persistent surface residual representation |
...
v52 source-config W&B rerun 已经启动，用于把当前 report-only effective policy 从 materialized v48/v51 结果推进到可复跑的 GPU render/eval 证据。
...
| 当前状态 | running；正在执行 `bicycle` v48 source apply |
| GPU | `CUDA_VISIBLE_DEVICES=4` |
| 当前空间占用 | 约 `852M` in `/dev/shm` |
| 完成后目标 | 生成 fresh source-rerun outputs、metrics、logs，并决定是否 promotion 到 v52 selected artifact tree |

这部分不建议明天作为已完成结果讲；可以说“source-config rerun is running to close the reproducibility gap”。
...
| v52 running source-rerun root | `/dev/shm/peilincai_spcarnet_v52_source_rerun_20260623_144849` |
...
| 本报告 | `docs/car_model/6-23-SPCarNet-Mentor-PPT-Technical-Report-v18.zh.md` |
