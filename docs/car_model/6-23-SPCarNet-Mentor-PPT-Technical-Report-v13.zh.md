...
当前结论状态：`NOT COMPLETE`。Phase-J 是当前最可靠、最适合汇报的强结果；v48 是当前最完整的 representation-level 内化证据；v49b/v50 是公平性和风险 gate 诊断，不应作为 headline。
...
| 最新表示级结果 | v48 surface residual atlas 相对 same-evidence no-op：`7 / 9` strict，`8 / 9` non-regressive/tie，mean `+0.001462` PSNR，`+0.00002774` SSIM，`-0.00003953` LPIPS |
| 最新安全诊断 | v50 fair-noop/L1-risk locked policy 相对 no-op：`6 / 9` strict，mean `+0.001264` PSNR，`+0.00002174` SSIM，`-0.00003405` LPIPS |

汇报主线建议：

> 我们已经得到一条在本地强 MeshSplatting baseline 上 full9 全场景严格胜出的 compact + repair pipeline。当前最需要继续突破的是把 render-time ELA 的大收益进一步内化成 persistent surface representation。
...
| 正向但非 headline | v48 train-only auto-support surface residual atlas | 作为 representation-level 内化方向和消融进展 |
...
| Internalization | surface residual atlas v42/v46/v47/v48 | 将 residual repair 推向 surface-addressed persistent 表示 | v48 是当前正向证据 |
...
| support/capacity 受限 | `counter/kitchen/bonsai` 触及 v48 `2048` extra-face cap | 下一阶段做 support ladder / higher-capacity atlas |
...
- v51 support ladder 方向仍属于未验证开发/诊断，不应在 mentor 汇报中写成结果；
...
| 12 | Ablation / Lessons | Phase-J strong；v48 direction right but effect small；v50 fixes fairness/gate diagnosis |
...
| this report | `docs/car_model/6-23-SPCarNet-Mentor-PPT-Technical-Report-v13.zh.md` |
