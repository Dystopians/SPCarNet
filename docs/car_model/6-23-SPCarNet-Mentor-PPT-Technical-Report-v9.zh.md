# SPCarNet 当前方法完整技术报告 v9 / v48 refresh
...
项目状态：`NOT COMPLETE`。Phase-J 是当前强阶段性结果；v48 是最新 representation-level 内化里程碑，但还没有取代 Phase-J，也不能说已经形成最终顶会闭环。
...
| 最新表示级进展 | v48 full9 已补齐：相对 same-evidence no-op compact 为 `7 / 9` 三指标严格胜出、`8 / 9` non-regressive/tie，mean gain `+0.001462` PSNR、`+0.00002774` SSIM、`-0.00003953` LPIPS；四场景核心子集仍为 `4 / 4` strict |
| 诚实边界 | 最强外观收益仍主要来自 render-time ELA；representation-level atlas full9 为小幅正收益，但 `stump` 被 gate 回退 no-op，`treehill` LPIPS 轻微回退 |
...
> 原始 MeshSplatting 是“训练出网格后直接渲染”。SPCarNet 是“训练出网格后，再用训练视角证据判断哪里能安全删、哪里容易错、哪里有可靠 residual 可以修；证据不足时自动回退”。当前 Phase-J 在 full9 上已经稳定超过强本地 baseline，v48 进一步把 residual 修复推进到可自扩展 support 的 surface-addressed 表示，但下一步还需要把这种内化收益做大。
...
| Internalization | surface residual atlas v42/v46/v47/v48 | 尝试把 residual repair 写进 surface-addressed 表示 | v48 是最新进展，但不是 headline endpoint |
...
v46 解决 v43 的问题：固定 nearest-observed empty-bin fill 在 garden/bonsai 上有利，但会损害 room/counter 的 v42 表现；如果靠人工挑 fill mode，会变成参数游戏。
...
结论：v46 是更合理的自动策略接口，不再依赖人工挑 fill mode；它稳定超过 no-op，均值略强于 v42，但相对 v42 并不是全场景三指标 strict win。

### 3.7 v47 Auto-Capacity Guarded Atlas

v47 是 v46 之后的表示级改动。它在 v46 的 auto-fill 之上新增 train-only 表示容量选择，解决“固定 texture size 导致 atlas 容量不足或过拟合风险不可控”的问题。

固定候选：

```text
texture_size in {8, 16, 24, 32}
fill mode in {face_mean, nearest_observed}
selection evidence: train policy-val only
promotion guard: relative gain, image SSIM gain, CVaR20, min-view gain
...
v47 的核心变化是联合选择 `(texture_size, fill_mode)`，而不是人工为不同场景指定参数。它把 v42/v46 的风险门控保留下来：只有 train policy-val 证据显示非退化，才允许提高 atlas 容量；否则回退到更保守的候选。
...
结论：v47 是一个真实方法改动，不是手调参数。它让 `garden/counter` 自动提升到更高 atlas capacity，同时让 `room/bonsai` 回退或保持安全候选。它相对 no-op、v42、v43、v46 的四场景均值都更好，但绝对收益仍是微小量级，所以适合放在“representation-level internalization progress / ablation”页，不适合替代 Phase-J 做主结果。v48 已经进一步替代 v47 成为最新表示级结果。

### 3.8 v48 Auto-Support Auto-Capacity Guarded Atlas

v48 针对 v47 的主要瓶颈：atlas 只能修改 target ray 落在 carrier-supported faces 上的像素，而 v47 的 target support coverage 很低。v48 不再只调 atlas texture capacity，而是加入 train-only support expansion：
...
v48 four-scene policy decision：

| scene | selected support | +faces | texture | fill | alpha | changed fraction |
|---|---|---:|---:|---|---:|---:|
| garden | fit_residual_topk | 977 | 16 | nearest_observed | 0.1250 | 1.8491% |
| room | base_carrier | 0 | 16 | face_mean | 0.1250 | 1.0602% |
| counter | fit_residual_topk | 2048 | 32 | nearest_observed | 0.1250 | 4.2312% |
| bonsai | fit_residual_topk | 2048 | 16 | nearest_observed | 0.1250 | 2.0578% |

v48 four-scene held-out metrics：

| scene | v48 PSNR | v48 SSIM | v48 LPIPS | dPSNR vs no-op | dSSIM | dLPIPS | dPSNR vs v47 | dSSIM | dLPIPS |
|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|
| garden | 24.742157 | 0.75409001 | 0.24797200 | +0.001154 | +0.00004101 | -0.00005122 | +0.000895 | +0.00003618 | -0.00003994 |
| room | 28.740660 | 0.88482928 | 0.24989747 | +0.001656 | +0.00003928 | -0.00001849 | +0.000000 | +0.00000000 | +0.00000000 |
| counter | 26.753004 | 0.86208355 | 0.25191796 | +0.003168 | +0.00003421 | -0.00008002 | +0.001593 | +0.00002706 | -0.00005126 |
| bonsai | 28.868425 | 0.89606690 | 0.25924534 | +0.004045 | +0.00005686 | -0.00008827 | +0.002874 | +0.00005275 | -0.00007921 |
...
结论：v48 是当前最好的 representation-level atlas 证据。它直接解决 v47 的 support bottleneck，在 `garden/counter/bonsai` 自动接受 expanded support，在 `room` 安全回退到 base carrier，因此比 v47 更像一个真实应用中的智能决策策略。但它的绝对收益仍是小幅稳定收益，不能替代 Phase-J 做主结果。

v48 full9 extension：

| scene | support | +faces | texture | fill | alpha | changed | dPSNR vs no-op | dSSIM | dLPIPS | strict |
|---|---|---:|---:|---|---:|---:|---:|---:|---:|---:|
| bicycle | fit_residual_topk | 1453 | 24 | nearest_observed | 0.1250 | 0.8947% | +0.000536 | +0.00000691 | -0.00000879 | 1 |
| flowers | fit_residual_topk | 1961 | 32 | nearest_observed | 0.0312 | 2.0957% | +0.000137 | +0.00000477 | -0.00000215 | 1 |
| garden | fit_residual_topk | 977 | 16 | nearest_observed | 0.1250 | 1.8491% | +0.001154 | +0.00004101 | -0.00005122 | 1 |
| stump | fallback no-op | 0 | 8 | face_mean | 0.1250 | 0.0000% | +0.000000 | +0.00000000 | +0.00000000 | 0 |
| treehill | fit_residual_topk | 933 | 24 | nearest_observed | 0.0312 | 1.3127% | +0.000195 | +0.00000209 | +0.00001916 | 0 |
| room | base_carrier | 0 | 16 | face_mean | 0.1250 | 1.0602% | +0.001656 | +0.00003928 | -0.00001849 | 1 |
| counter | fit_residual_topk | 2048 | 32 | nearest_observed | 0.1250 | 4.2312% | +0.003168 | +0.00003421 | -0.00008002 | 1 |
| kitchen | fit_residual_topk | 2048 | 24 | nearest_observed | 0.1250 | 2.9758% | +0.002270 | +0.00006449 | -0.00012597 | 1 |
| bonsai | fit_residual_topk | 2048 | 16 | nearest_observed | 0.1250 | 2.0578% | +0.004045 | +0.00005686 | -0.00008827 | 1 |
...
full9 结论要比四场景版本更谨慎：`stump` 被 train policy-val gate 拒绝并回退 no-op，`treehill` 虽然 PSNR/SSIM 微正但 LPIPS 回退 `+0.00001916`。因此 v48 full9 证明了 surface-addressed residual atlas 的安全性和自动回退机制，但还没有形成足够强的视觉主结果。

## 4. 训练与评估流程

需要向 mentor 讲清楚：SPCarNet 不是从零训练一个新网络，而是在 MeshSplatting 训练后进行 evidence-certified post-training optimization。
...
中量/长程训练与评估在需要时使用 W&B online logging。当前这份报告中的 Phase-J 主结果和 v42/v46/v47/v48 表示级结果均已有本地落盘 JSON/Markdown 审计路径。v48 是 eval/apply 级别的真实 train/eval pipeline 改动，不是完整长程重新训练。
...
| v47 auto-capacity policy | train-only 联合选择 atlas texture size 和 fill mode | 比 no-op/v42/v43/v46 的四场景均值都更好；是 v48 之前最完整的 atlas 策略 |
| v48 auto-support policy | train-only support expansion + auto-capacity + auto-fill + non-regression guard | 当前最完整的 representation-level atlas 策略；full9 为 7/9 strict、8/9 non-regressive/tie vs no-op，四场景核心子集 4/4 strict；stump 自动回退，treehill LPIPS 轻微回退，effect size 仍小 |
...
| representation-level 收益小 | v48 full9 相对 no-op 7/9 strict、8/9 non-regressive/tie，均值只有 +0.00146 PSNR；四场景核心子集相对 v47 均值 +0.00134 PSNR | 放在 next-step/ablation，不做 headline |
...
9. **Ablation / Recent Progress**
   v48 auto-support surface residual atlas：真实表示级推进，full9 7/9 strict vs no-op、8/9 non-regressive/tie；四场景核心子集 4/4 vs no-op，3/4 strict vs v47。

10. **Limitations and Next Step**
    最大短板是 residual repair 还没完全强力内化；下一步是 high-capacity surface residual representation。
...
| v47 method log | `docs/car_model/6-23-v47-AutoCapacitySurfaceAtlas-Log.md` |
| v47 summary JSON | `outputs/carnet/meshsplatopt/ecsr_phase_v39_multiscene_ssimaware/v47_autocap_guarded_v42calib_multiscene_summary.json` |
| v48 method log | `docs/car_model/6-23-v48-AutoSupportSurfaceAtlas-Log.md` |
| v48 four-scene summary JSON | `outputs/carnet/meshsplatopt/ecsr_phase_v39_multiscene_ssimaware/v48_autosupport_autocap_guarded_v42calib_multiscene_summary.json` |
| v48 four-scene summary MD | `outputs/carnet/meshsplatopt/ecsr_phase_v39_multiscene_ssimaware/v48_autosupport_autocap_guarded_v42calib_multiscene_summary.md` |
...
| v47 implementation | `scripts/car_model/ecsr_apply_surface_residual_region_texture_adapter.py` |
| this report | `docs/car_model/6-23-SPCarNet-Mentor-PPT-Technical-Report-v9.zh.md` |
