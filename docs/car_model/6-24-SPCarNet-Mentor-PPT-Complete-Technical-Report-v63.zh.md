...
| v63/v63b bin alpha calibration | 最新真实方法改造 | kitchen 上首次严格超过 v52/v60，counter 仍未过关键 reference；不能包装为全局主结果 |

推荐开场：

> 我们已经有一个能公平击败本地 clean MeshSplatting baseline 的 Phase-J endpoint，并且同时减少 triangles。它足够作为当前主线汇报。与此同时，论文终局仍要把 Phase-J 的 render-time repair 进一步内化为 persistent surface residual representation。v48-v63 是这条内化路线的连续实验、接口建设和风险诊断。
...
> v63b 是一个有价值的新里程碑，因为它首次在 kitchen 上把表示级 residual atlas 推到严格三指标超过 v52/v60；但它没有解决 counter 上的关键 reference 差距，所以不能宣称全局 promotion。合理的下一步不是继续手工挑场景，而是把 v63b 的 policy-val 信号变成固定 auto-select/auto-reject policy，并在 full9 fresh run 上验证。
...
- v59-v63 probe 使用 W&B online logging，保留 command/config/result path/error；
...
| 最新 v63b 是不是最好？ | 不是全局最好。它在 kitchen 上严格超过 v52/v60，但 counter 仍低于关键 reference。 |
| 如果全图看不明显怎么办？ | 展示 crop/error map。定量上 full9 9/9 strict、244/246 per-view strict；定性上需要看局部残差修复。 |
| out-of-trajectory 会不会崩？ | Phase-J 有 risk gate 和 fallback/no-op；v60-v63 进一步证明 persistent residual field 需要更强 uncertainty / calibration。 |
...
| 短板 | 影响 | 当前结论 |
|---|---|---|
| Phase-J 最强收益来自 render-time ELA | 可能被质疑为后处理 | 汇报时承认；把 v48-v63 作为内化路线 |
| 表示级 atlas 收益太小 | 很难替代 Phase-J headline | v52/v56/v63b 可做 ablation，不做主 claim |
...
1. 把 v63b 的 policy-val 信号固化成全自动 policy：只有 train-only evidence 足够强时启用 bin-alpha residual field，否则回退 v52/v56。
...
4. 在 full9 fresh run 中验证 fixed policy，避免 per-scene/manual selection。
...
| 14 | Latest Method Probe | v63/v63b bin alpha calibration：kitchen 成功、counter 未闭合 |
...
> SPCarNet 将 MeshSplatting 从“训练完成后直接渲染”的静态网格，升级成“训练证据驱动的可压缩、可修复、可回退表示”。我们从训练视角挖掘 surface evidence，判断哪些三角形可以安全删除，哪些局部 residual 可以迁移修复，哪些区域因为证据不足必须保持原样。当前 Phase-J endpoint 在 Mip-NeRF360 full9 上相对本地强 clean MeshSplatting baseline 达成 `9/9` 场景 PSNR/SSIM/LPIPS 严格胜出，同时平均减少 `7.65%` triangles。最新 v63/v63b 将表示级 residual atlas 从二值 allowlist 推进到 bin-level residual magnitude calibration，已在 kitchen 上得到严格三指标提升，但 counter 仍未越过关键 reference，因此后续需要固定自动策略和更强 uncertainty-certified residual field。

英文摘要：

> SPCarNet upgrades MeshSplatting from a static trained mesh into a training-evidence-certified compact and repairable representation. Given a clean MeshSplatting checkpoint, we mine training-view surface evidence, remove low-risk redundant triangles, and transfer reliable residual appearance cues through a guarded residual adapter. All repair decisions are driven by train/policy-validation evidence, while held-out test views are used only for reporting. On Mip-NeRF360 full9, the current Phase-J endpoint strictly improves PSNR, SSIM, and LPIPS over a strong locally selected clean MeshSplatting baseline on all 9 scenes, with 244/246 per-view strict wins and 7.65% average triangle reduction. The latest v63/v63b branch further studies persistent surface residual fields through bin-level residual magnitude calibration, showing a strict kitchen improvement while still exposing unresolved counter-scene generalization limits.
...
```text
docs/car_model/6-24-SPCarNet-Mentor-PPT-Complete-Technical-Report-v63.zh.md
```
...
> 我们已经有一个清楚击败本地 clean MeshSplatting baseline 的 Phase-J endpoint，并且它同时带来 RGB 指标提升和 triangle reduction。这个结果足够作为当前方法主线向 mentor 汇报。与此同时，我们必须承认 Phase-J 的最强外观收益仍来自 render-time ELA，所以论文终局还需要把它进一步内化成 persistent surface representation。v48-v63 是这条路线的连续实验，目前证明了 evidence interface、审计、W&B 验证、安全 guard、view-conditioned basis 和 bin-level magnitude calibration 都可行；但 counter 上仍有关键 reference 没超过，说明下一步必须做 fixed auto policy 和 uncertainty-certified residual field，而不是继续手工挑场景或扫阈值。
