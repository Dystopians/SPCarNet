# SPCarNet 当前方法完整技术报告 v19

日期：2026-06-23 16:15 -0700

用途：mentor 汇报、PPT 制作、方法交底、后续论文路线讨论。

当前主讲 endpoint：`ours_26000_phasej_guarded_adaptedge_ela`

当前项目状态：`NOT COMPLETE`。Phase-J 是当前最可靠、最适合放在主结果页的强结果；v48/v52 是把修复从 render-time adapter 推向 persistent surface representation 的最新证据，但还不能包装成已经替代 Phase-J 的最终论文方法。v52 source-config W&B rerun 已完成，fresh source-config rerun 复现 `9 / 9` 场景，`0` missing，`0` metric mismatch。

---

## 0. 一页结论

SPCarNet 的核心目标不是从零替代 MeshSplatting，而是在一个已经训练好的 MeshSplatting checkpoint 上增加训练证据驱动的压缩、修复和风险控制阶段。
...
| v52 source-config 复现 | W&B rerun 已完成；fresh source-config audit 完成 `9 / 9`，`0` missing，`0` mismatch |
...
| 等级 | 内容 | 应该怎么讲 |
|---|---|---|
| 主结果 | Phase-J guarded adaptive ELA full9 | 现在最稳的 headline：full9 `9 / 9` strict RGB wins vs selected clean MeshSplatting |
| 表示级证据 | v48 auto-support surface residual atlas | 证明 residual repair 可以变成 train-only surface-addressed representation，但 effect size 还小 |
| 最新策略整合 | v52 capacity-aware v48/v51 policy | 把 v51 的 support-capacity 收益只用在 cap-hit 场景，避免全局固定参数伤害 v48 |
| 不适合主讲成最终成果 | v49b/v50/v51 单独结果 | 适合作为 ablation、失败诊断和设计依据 |
| 暂时不能声称 | 已经 100% 顶会终局、v52 已替代 Phase-J、严格同协议超过原论文表格 | 当前证据不够，容易被追问 fairness 和 reproducibility |
...
MeshSplatting 的价值在于它输出 triangle mesh。相比纯 image-space 方法、点云或 Gaussian 表示，mesh 更容易进入传统图形管线、游戏引擎、AR/VR、数字孪生和几何编辑流程。
...
| Internalization | surface residual atlas v48/v51/v52 | 将 residual repair 推向 surface-addressed persistent 表示 | 当前仍是正向但细微的表示级证据 |
...
当前 Phase-J 平均 triangle reduction 是 `7.6479%`。这里的 triangle reduction 是“删去的三角形占比”，不是“剩余比例”。PPT 中建议称为“quality-first geometry simplification”，不要称为“极限压缩”。
...
- 诚实边界是：source-config GPU rerun 已复现全 9 场景，但 v52 的 effect size 仍然很小，不能作为最终论文 endpoint。
...
| 14 | Risks | render-time adapter、定性 subtle、v52 effect size 仍小 |
| 15 | Next Step | 推进高容量 persistent surface residual representation；让表示级收益更明显 |
...
v52 source-config W&B rerun 已完成，用于把当前 effective policy 从 materialized v48/v51 结果推进到可复跑的 GPU render/eval 证据。
...
并行加速命令覆盖了剩余场景，均使用 W&B logging 和 `/dev/shm` 输出根目录，避免继续占满 `/data`。`stump` 的 rejected/no-op 分支暴露了原先没有写 metrics 的问题，已通过 planner 自动追加 `--write_noop_on_reject` 修复。

当前状态：

| 项目 | 值 |
|---|---|
| W&B main run | `https://wandb.ai/karamazovaniki-university-of-southern-california/spcarnet/runs/2j5osvgg` |
| W&B supplemental runs | `counter=9uwsu9m1`, `kitchen=b9w1zonu`, `bonsai=uh4a9hvu`, `room=o785gymj`, `stump=n4ucitf6` |
| main output root | `/dev/shm/peilincai_spcarnet_v52_source_rerun_20260623_144849` |
| parallel roots | `/dev/shm/peilincai_spcarnet_v52_source_rerun_parallel_*_20260623` |
| fresh source-config audit | `9 / 9` completed，`0` missing，`0` mismatches |
| reproduced scenes | `bicycle`, `flowers`, `garden`, `stump`, `treehill`, `room`, `counter`, `kitchen`, `bonsai` |
| 当前判断 | 可以作为 v52 表示级 ablation 的 full9 source-rerun evidence |
...
这部分明天可以谨慎讲成 “v52 source-config rerun closes the reproducibility gap for the representation-level ablation”。不要讲成最终论文 endpoint，因为 v52 的 effect size 仍然很小。
...
| v52 source-rerun root | `/dev/shm/peilincai_spcarnet_v52_source_rerun_20260623_144849` |
...
| 本报告 | `docs/car_model/6-23-SPCarNet-Mentor-PPT-Technical-Report-v19.zh.md` |
