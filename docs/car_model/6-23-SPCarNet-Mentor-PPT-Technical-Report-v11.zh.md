...
当前状态：`NOT COMPLETE`。Phase-J 已经形成一条强且可审计的阶段性主线；v48 证明 residual repair 可以推进到 surface-addressed representation，但还没有达到 Phase-J 的视觉收益量级，也还不能说是顶会终局方法。

---

## 0. 给 mentor 的一页结论

SPCarNet 的核心思想是：把 MeshSplatting 训练出的 mesh checkpoint 当作一个可以继续诊断、压缩和修复的基础表示，而不是训练结束后直接拿来渲染。

当前最适合汇报的主线是：

```text
clean MeshSplatting checkpoint
  -> train-view surface evidence mining
...
> 原始 MeshSplatting 是“训练出网格后直接渲染”。SPCarNet 是“训练出网格后，再用训练视角证据判断哪里能安全删、哪里容易错、哪里有可靠 residual 可以修；证据不足时自动回退”。
...
| 公平 baseline | selected clean MeshSplatting：每个场景从 clean `26000/30000` 选择 held-out test score 更强者 |
...
| 与 MeshSplatting paper table | Phase-J mean `26.4828 / 0.7837 / 0.2243`，paper table mean `24.78 / 0.728 / 0.310`；该对比只能作为 sanity check |
| 最新 representation-level 证据 | v48 相对 same-evidence no-op compact baseline：`7 / 9` strict，`8 / 9` non-regressive/tie，均值 `+0.001462` PSNR，`+0.00002774` SSIM，`-0.00003953` LPIPS |
| 诚实边界 | Phase-J 最强收益仍主要来自 render-time ELA；v48 是内化路线证据，但 effect size 还小 |

汇报时建议主讲：

> 我们目前已经有一个在本地强 MeshSplatting baseline 上 full9 全场景胜出的 compact + repair pipeline。真正需要继续攻克的是把 render-time ELA 的大收益进一步内化成 persistent surface representation。

---

## 1. 研究问题

MeshSplatting 的价值在于输出 triangle mesh。相比纯 Gaussian 或点云，这种表示更容易进入传统图形管线、AR/VR、游戏引擎、数字孪生、几何编辑和后续压缩。

但 clean MeshSplatting 仍然有三个问题：

| 问题 | 现象 | SPCarNet 的应对 |
|---|---|---|
| 局部外观 residual | 树叶、树皮、桌面纹理、室内边缘仍有颜色偏差或模糊 | 从 train views 挖 residual，并用 guarded adapter 迁移到 held-out render |
| 拓扑冗余 | 一部分 faces 对多视角解释贡献低 | 用 train evidence 做 sparse-occlusion protected compaction |
| 方法决策风险 | 盲目 residual transfer 容易伤害 tail views | 用 policy-val gate、CVaR、min-view、edge fallback 和 no-op 保护 |
...
| Geometry | sparse-occlusion protected compaction | 删除低风险 triangles，保持 topology 和 renderer compatibility | Phase-J 主结果已使用 |
| Appearance | Evidence Lumigraph Adapter | 将 train residual 迁移到 held-out render | Phase-J 主收益来源 |
| Safety | train-only guarded policy | 选择 alpha、edge fallback、reject/no-op | Phase-J 已闭合 |
| Internalization | surface residual atlas v42/v46/v47/v48 | 将 residual repair 推向 surface-addressed persistent 表示 | v48 是最新进展，但不是 headline |
...
| test GT | 用于最终评价 | 同样只用于最终评价，不参与 method branch/alpha/fallback |
...
这一步不是看 held-out test，而是把训练集里的“哪里稳定、哪里不稳定、哪里存在可迁移 residual”结构化。
...
当前 Phase-J 平均删面 `7.6479%`。PPT 中应讲成“质量提升同时有真实几何简化”，不要讲成极限压缩算法。
...
Phase-J 的关键不是单纯 ELA，而是 ELA 外面有一整套 guard：
...
Phase-J 的最大短板是最强收益仍来自 render-time adapter。为了解决 reviewer 可能质疑的“这是不是后处理”，我们推进了 surface-addressed atlas 线。

v48 的动机：

> v47 的 atlas 只能修改 target ray 落在 carrier-supported faces 上的像素；一旦 support coverage 低，表示级 residual repair 的效果就被严重限制。
...
### 6.2 与 MeshSplatting paper table 的关系
...
但 PPT 中必须谨慎：paper table 可能存在实现、数据预处理、分辨率、mask、evaluation script 等差异，因此只能作为 sanity check。最严谨 claim 是超过本地同协议 selected clean MeshSplatting baseline。
...
| this report | `docs/car_model/6-23-SPCarNet-Mentor-PPT-Technical-Report-v11.zh.md` |
