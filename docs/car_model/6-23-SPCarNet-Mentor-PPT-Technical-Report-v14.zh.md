...
当前结论状态：`NOT COMPLETE`。Phase-J 是当前最可靠、最适合汇报的强结果；v48 是当前最完整的 full9 representation-level 内化证据；v49b/v50 是公平性和风险 gate 诊断；v51 是刚完成的 support-footprint ladder 单场景正向 probe，尚未 full9 闭合，不应作为 headline。
...
| 最新开发探针 | v51-fast 在 `counter` 上启用 support-footprint ladder，选中 `fit_residual_topk_4096`，相对 v48 `+0.000456` PSNR、`+0.0000311` SSIM、`-0.0000496` LPIPS |

汇报主线建议：

> 我们已经得到一条在本地强 MeshSplatting baseline 上 full9 全场景严格胜出的 compact + repair pipeline。当前最需要继续突破的是把 render-time ELA 的大收益进一步内化成 persistent surface representation；v51 的单场景结果说明“扩大 surface support footprint”是下一步值得验证的方向。
...
| 正向但非 headline | v48 train-only auto-support surface residual atlas | 作为 full9 representation-level 内化方向和消融进展 |
| 诊断 / 修复 | v49b fair-noop fallback、v50 L1-risk gate | 作为严谨性、fairness、failure analysis |
| 最新 probe | v51-fast support-footprint ladder | 作为下一阶段实验动机，只能说 counter 单场景正向 |
...
| Internalization | surface residual atlas v42/v46/v47/v48/v51 | 将 residual repair 推向 surface-addressed persistent 表示 | v48 是当前 full9 正向证据；v51 是 support-capacity probe |
...
### 7.2 v51-fast Counter 正向结果

固定 `texture=32`、`fill=nearest_observed`、support candidates 为 `2048,4096` 后，`counter` smoke 成功完成：
...
| item | value |
|---|---:|
| selected support | `fit_residual_topk_4096` |
| support base faces | `1574` |
| support added faces | `4096` |
| support candidate faces | `5670` |
| fit views | `34` |
| policy-val views | `12` |
| selected texture | `32` |
| selected fill | `nearest_observed` |
| selected alpha | `0.125` |
| target changed fraction | `6.5362%` |
| target written views | `30` |
| policy-val relative gain | `0.021614` |
| policy-val SSIM gain | `0.000165328` |
| policy-val image L1 gain | `0.000013705` |
| policy-val L1 positive fraction | `0.916667` |

held-out test metrics on `counter`：

| row | PSNR | SSIM | LPIPS |
|---|---:|---:|---:|
| no-op compact baseline | 26.749836 | 0.86204934 | 0.25199798 |
| v48 auto-support atlas | 26.753004 | 0.86208355 | 0.25191796 |
| v50 L1-risk locked | 26.753008 | 0.86208361 | 0.25191796 |
| v51-fast support ladder | 26.753460 | 0.86211467 | 0.25186834 |

v51-fast deltas：

| comparison | dPSNR | dSSIM | dLPIPS |
|---|---:|---:|---:|
| v51-fast vs no-op | +0.003624 | +0.00006533 | -0.00012964 |
| v51-fast vs v48 | +0.000456 | +0.00003111 | -0.00004962 |
| v51-fast vs v50 | +0.000452 | +0.00003105 | -0.00004962 |
...
- 它首次证明 cap-hit 场景 `counter` 可以通过扩大 surface support footprint 进一步提升 v48/v50；
- 但它目前只有 `counter` 单场景完成，不能宣称 full9 全面提升；
- 下一步应固定这个 fast policy，在 `kitchen/bonsai` 等 cap-hit 场景优先补验证，再决定是否 full9 推广。
...
| v51-fast support ladder | 固定 texture/fill 后只做 support footprint ladder，允许 `2048/4096` extra-face candidates | `counter` 单场景相对 v48/v50/no-op 三指标提升；尚未 full9 |
...
| support/capacity 受限 | `counter/kitchen/bonsai` 触及 v48 `2048` extra-face cap；v51-fast 只在 `counter` 上验证正向 | 下一阶段固定 fast policy 补 `kitchen/bonsai/full9` |
...
- v51 support ladder 目前只能说是单场景开发 probe，不应说成 full9 已完成结果；
...
| 12 | Latest Probe | v51 support-footprint ladder 在 `counter` 上进一步超过 v48/v50 |
| 13 | Ablation / Lessons | Phase-J strong；v48 direction right but effect small；v50 fixes fairness/gate diagnosis |
| 14 | Risks | render-time adapter、定性 subtle、representation-level 还不够强 |
| 15 | Next Step | high-capacity persistent surface residual representation |
...
| v51-fast counter small artifacts | `outputs/carnet/meshsplatopt/ecsr_phase_v39_multiscene_ssimaware/v51_fast_support_ladder_small_artifacts_20260623/counter` |
...
| v51-fast counter results JSON | `outputs/carnet/meshsplatopt/ecsr_phase_v39_multiscene_ssimaware/v51_fast_support_ladder_small_artifacts_20260623/counter/results.json` |
...
| this report | `docs/car_model/6-23-SPCarNet-Mentor-PPT-Technical-Report-v14.zh.md` |
...
>
> 目前最大的诚实短板是 Phase-J 的主要收益仍来自 render-time Evidence Lumigraph Adapter。为了解决这个问题，我们已经推进了 v48 surface residual atlas，把 residual repair 转成 train-only 自扩展 support 的 surface-addressed 表示。v48 full9 相对 no-op compact baseline 有 `7/9` strict、`8/9` non-regressive/tie，但收益量级还小。v49b/v50 进一步修正了 fair fallback 和 L1-risk gate：v50 能把 `room/counter` 从过拒绝中恢复，但 full9 仍弱于 v48。最新 v51-fast 在 `counter` 上用 support-footprint ladder 选中 `4096` extra faces，并进一步超过 v48/v50/no-op，说明 support capacity 是值得继续推进的瓶颈方向。因此明天可以把 Phase-J 作为主结果，把 v48 作为 representation-level 主证据，把 v51 作为下一阶段 probe，把 v49b/v50 放在 ablation/lessons 中。
...
> 当前工作已经有一条可信主线：Phase-J 在本地强 MeshSplatting baseline 上 full9 全场景严格胜出，同时有真实删面、per-view 审计和 geometry-safe 证据。它足够作为阶段性强结果汇报。最需要继续攻克的是 representation-level 内化：v48 已经证明 surface-addressed residual atlas 可以用 train-only policy 做 support/容量自适应，并在 full9 上取得均值正收益；但它仍有 `stump` 自动回退和 `treehill` LPIPS 轻微回退，effect size 远小于 Phase-J。v51-fast 进一步说明 cap-hit 场景可以通过更大的 support footprint 获得增益，但目前只有 `counter` 单场景，必须补 `kitchen/bonsai/full9` 后才可升级为主结论。下一阶段应该集中做高容量、可持久化的 surface residual representation，而不是继续做微小参数扫描。
