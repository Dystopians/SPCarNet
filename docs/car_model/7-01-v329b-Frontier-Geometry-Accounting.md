...
Important protocol note: v329b inherits the same compact parent topology as the current support-transport frontier. The v329b policy changes render/color selection, not the mesh triangle or vertex count, so this ledger is the geometry accounting for the current v329b evidence package.
