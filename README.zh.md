...
**当前闭环状态，2026-06-24：** `NOT COMPLETE`。当前最强可审计 endpoint 仍是 Phase-J：在本地复现的 selected-clean `26000/30000` envelope、相同 full9 split 和相同 evaluator 下，scene-level PSNR/SSIM/LPIPS 为 `9 / 9` strict，per-view RGB 为 `244 / 246` strict，平均删面 `7.6479%`。当前最佳固定 representation-level policy 是 v64 fixed auto bin-alpha：相对 v56 为 `9 / 9` non-regressive/tie，只提升 `kitchen`，mean gain 仍只有 `+0.000410080` PSNR、`+0.000000278` SSIM、`-0.000018951` LPIPS。v65 teacher basis 和 v66 bin-RGB alpha 是已经完成的负结果诊断，不是 promoted endpoint。当前报告：[`Mentor/PPT 技术报告 v66`](docs/car_model/6-24-SPCarNet-Mentor-PPT-Complete-Technical-Report-v66.zh.md)；v66 持久证据：`outputs/carnet/meshsplatopt/ecsr_phase_v39_multiscene_ssimaware/v66_bin_rgb_alpha_probe_20260624/summary.md`。
...
**v57a reliability-shrink 探针，2026-06-24：** `REAL_PIPELINE_CHANGE_UNDER_PROBE_NOT_PROMOTED`。v57a 在 atlas adapter 和 runner 中加入 train-policy-val face-alpha reliability shrink。首个 `counter` W&B probe 相对 v52 仍正向（`+0.001548767` PSNR、`+0.000009656` SSIM、`-0.000117481` LPIPS），但弱于 raw v55d（`-0.001121521` PSNR、`-0.000001907` SSIM、`+0.000059486` LPIPS）。该实现是有用接口，但不能按当前参数推广；`kitchen` SSIM-risk 验证正在运行。证据：[`v57a face-alpha reliability shrink 探针`](docs/car_model/6-24-v57a-FaceAlphaReliabilityShrink-Probe-Log.md)。
