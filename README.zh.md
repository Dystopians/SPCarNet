...
| **F75** | **自适应 + 稀疏 + LPIPS λ = 0.00025（parking compact base）** | **`ACCEPTED_FOR_PARKING_COMPACT_BASE`** —— 在所有 compact-checkpoint 指标上超过 R53.01 / F7；parking RGB 渲染头条由 OUT2 接替 |
...
### 严格公平 baseline 收束（ELA7–ELA12 + OUT2）
...
### Parking 深度对比（R44 → R53 → F75 → OUT2）

parking 场景同时维护一条单场景线，承载失败证据骨架（R44 vs clean 22k）、ELA 之前最强 compact checkpoint（F75）和当前室外视觉修复（OUT2-v6）。每行一个保留测试视图；列依次为 GT、clean-long 22k / 30k、R48、R53。
...
三条 recipe 已验证。**Recipe A —— CSEF 家族 + validation-budget（F49）** 是跨场景头条：5 / 5 场景在渲染与稀疏深度指标上击败 clean-long。**Recipe B —— 自适应策略 + 极小 LPIPS（F75）** 是 OUT2 之前最强的 parking compact checkpoint，并从 checkpoint 中选择 prune 比例而非依赖手动调参表。**Recipe C —— 稀疏深度低 λ 恢复（R44）** 仍是跨场景的基础 recipe 与极低拓扑 Pareto 点，但在 parking 渲染上输给 clean 22k。**OUT2 local parent-gated ELA** 是当前 ELA12 使用的 parking 渲染修复层，用来处理室外定性长尾。
