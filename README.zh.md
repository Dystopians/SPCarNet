...
**Paper-loop 状态，2026-05-14：** `NOT COMPLETE`。Phase-J 是当前最强 endpoint：clean-best 与 Phase-J RGB 行在 `9 / 9` 场景完整，且 Phase-J 相对 selected clean MeshSplatting 在 `9 / 9` 场景三指标严格胜出。Phase-S 现在是一个真实的 representation-level face-local repair 分支，并完成了 risk-tail 全 8 个有候选场景 replay：接受 `3 / 8`，相对 Phase-J fallback 的 mean effective report-only delta 为 `+0.000684500` PSNR、`+0.000058956` SSIM、`-0.000073545` LPIPS。GeoRisk/CVaR selector 加入几何邻域排序与 train-val CVaR 诊断，但在本轮 7 个 hard/control 场景 replay 中只接受 `2 / 7`，没有扩大到超过原 risk-tail 的覆盖范围。最新 PatchRisk/direct PatchCert 进一步加入显式 patch carrier 与 per-view tail gate：PatchRisk 接受 `1 / 5`（`counter`），direct PatchCert 接受 `1 / 5`（`bicycle`），相对 Phase-J fallback 的 mean effective delta 为 `+0.000077` PSNR、`+0.000007` SSIM、`-0.000023` LPIPS。这是真实的方法里程碑，也是第一个被接受的 hard-scene patch-certified representation edit，但仍然不是最终论文 endpoint。最新模块/证据日志：[`Direct PatchCert`](docs/car_model/5-14-PhaseS-DirectPatchCert-Carrier-Pilot.md)、[`PatchRisk`](docs/car_model/5-14-PhaseS-PatchRisk-Carrier-Pilot.md)、[`GeoRisk/CVaR`](docs/car_model/5-14-PhaseS-GeoRiskCVaR-Selector-Log.md)、[`risk-tail/alpha`](docs/car_model/5-14-PhaseS-RiskTail-Alpha-ModuleLog.md)。
...
PatchCert 更强：固定 5 场景 replay 中接受 `bicycle`，但 `flowers` 仍是关键未解问题，
因为 test 指标明显为正而 train-val mean/tail gate 拒绝提升。
...
| Phase-S PatchRisk / direct PatchCert carrier | PatchRisk strict 5 场景 replay 接受 `1 / 5`（`counter`），均值 `+0.000014877` PSNR，`+0.000000072` SSIM，`-0.000000089` LPIPS；direct PatchCert 固定 5 场景 replay 接受 `1 / 5`（`bicycle`），相对 Phase-J fallback 的 mean effective 为 `+0.000077` PSNR，`+0.000007` SSIM，`-0.000023` LPIPS；定性图在 `outputs/carnet/meshsplatopt/ecsr_phase_s/phase_s_patchcert_v5_patchcarrier_pilot_20260514_qualitative` |
