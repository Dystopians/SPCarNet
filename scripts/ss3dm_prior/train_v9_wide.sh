...
OUTPUT_DIR="${OUTPUT_DIR:-$ROOT/outputs/ss3dm_prior/train_v9_wide}"
WANDB_MODE="${WANDB_MODE:-offline}"

python -m ss3dm_prior.train \
  --data_config "$ROOT/configs/ss3dm_prior/data_default.yaml" \
  --model_config "$ROOT/configs/ss3dm_prior/model_v9_wide.yaml" \
  --train_config "$ROOT/configs/ss3dm_prior/train_v9_wide.yaml" \
