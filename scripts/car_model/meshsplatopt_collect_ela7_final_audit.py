...
def _write_report(path: Path, payload: dict[str, Any]) -> None:
    ...
    lines = [
        ...
        "",
        "ELA7 remains promoted only for RGB render metrics. It beats the best pure Mesh Splatting clean baseline available in these selected scenes on PSNR, SSIM, and LPIPS under independent `metrics.py` outputs. It is not a full multi-axis win because it is renderer-side and does not improve geometry or reduce triangle count. The attempted ELA8 checkpoint distillation rows are not promoted unless listed as `PASS_PROMOTE` below.",
        "",
    ...
    lines.extend(
        [
            ...
            "## Interpretation",
            "",
            "The main unresolved weakness is not whether ELA7 beats the selected-scene clean render baseline; it does on RGB. The weakness is method form: ELA7 is still a renderer-side evidence portfolio, not a distilled checkpoint or compact topology method. It inherits clean geometry and clean topology, so it cannot satisfy a strict full claim that also requires geometry and triangle-count wins. The first ELA8 distillation pilot regressed below clean, which means persistent distillation needs a different mechanism before it can replace ELA7.",
        ]
    )
    path.parent.mkdir(parents=True, exist_ok=True)
    path.write_text("\n".join(lines) + "\n", encoding="utf-8")
...
def main() -> int:
    ...
    decision = (
        "ELA7_RGB_PROMOTED_ONLY_DISTILLATION_NOT_PROMOTED"
