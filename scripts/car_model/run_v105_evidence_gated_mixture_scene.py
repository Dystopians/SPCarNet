...
def _validate_v105_field_manifest(manifest: dict[str, Any], field: Path, args: argparse.Namespace) -> dict[str, bool]:
    checks = {
        ...
        "basis_type": str(manifest.get("basis_type", "")) == EXPECTED_BASIS_TYPE,
        "builder_variant": str(manifest.get("builder_variant", "")) == EXPECTED_BUILDER_VARIANT,
...
def run_scene(args: argparse.Namespace) -> dict[str, Any]:
    ...
    output_method = args.output_method or f"ours_{int(args.iteration)}_v105_{args.gate_source}_evidence_gated_mixture_{scene}"
    render_rc = 999
    render_wall = 0.0
    ...
    render_identity_checks = {
        ...
        "basis_type": str(render_field.get("basis_type", "")) == EXPECTED_BASIS_TYPE,
        "builder_variant": str(render_field.get("builder_variant", "")) == EXPECTED_BUILDER_VARIANT,
    ...
    payload = {
        ...
        "field_identity": {
            ...
            "expected": {
                ...
                "basis_type": EXPECTED_BASIS_TYPE,
                "builder_variant": EXPECTED_BUILDER_VARIANT,
        ...
        "field_stats": {
            ...
            "gate_source": solve_stats.get("gate_source"),
    ...
    out_json = report_root / f"{scene}_v105_evidence_gated_mixture_report.json"
    out_json.write_text(json.dumps(payload, indent=2, sort_keys=True) + "\n", encoding="utf-8")
    out_md = report_root / f"{scene}_v105_evidence_gated_mixture_report.md"
    out_md.write_text(
        "\n".join(
            [
                f"# v105 Evidence-Gated Mixture Report: {scene}",
                "",
