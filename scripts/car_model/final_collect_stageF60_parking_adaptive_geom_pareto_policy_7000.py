...
DOC = ROOT / "docs/car_model/final_stageF60_parking_adaptive_geom_pareto_policy_7000_report.md"
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--f60_root", default="outputs/carnet/meshprior/stageF60_parking_adaptive_geom_pareto_policy_7000/parking_7000iter_adaptive_geom_pareto_policy")
    ...
    parser.add_argument("--output_dir", default="outputs/carnet/meshprior/stageF60_parking_adaptive_geom_pareto_policy_7000/summary")
...
def main() -> None:
    ...
    rows = [
        ...
        helper._summarize("adaptive_geom_pareto_policy_F60", Path(args.f60_root), args.iteration, True),
        helper._summarize("no_gate_ratio004_7000", Path(args.nogate_root), args.iteration, False),
    ]
    refs = {row["label"]: row for row in rows}
    f60 = refs["adaptive_geom_pareto_policy_F60"]
    ...
    required = [f60, f58, f56, nogate, strict, calibrated, refs["delayed_ratio002_F53"], refs["delayed_ratio001_F54"]]
    payload = {
        "decision": "F60_COMPLETE" if all(row["results_ready"] for row in required) else "F60_IN_PROGRESS",
        "purpose": "adaptive CSEF policy evaluation against fixed-ratio parking rows and fair no-gate baselines",
        "rows": rows,
        "deltas_f60_minus_no_gate": _deltas(f60, nogate),
        "deltas_f60_minus_strict": _deltas(f60, strict),
        "deltas_f60_minus_calibrated": _deltas(f60, calibrated),
        "deltas_f60_minus_f58": _deltas(f60, f58),
        "deltas_f60_minus_f56": _deltas(f60, f56),
    }
    if payload["decision"] == "F60_COMPLETE" and _all_metric_win(payload["deltas_f60_minus_no_gate"]):
        if _all_metric_win(payload["deltas_f60_minus_f56"]):
            payload["interpretation"] = "F60 adaptive CSEF policy strictly beats both no-gate and the strongest fixed-ratio F56 on all six tracked metrics."
        else:
            payload["interpretation"] = "F60 adaptive CSEF policy strictly beats no-gate on all six tracked metrics but remains mixed versus F56."
    elif payload["decision"] == "F60_COMPLETE":
        payload["interpretation"] = "F60 is complete but still mixed versus no-gate; inspect deltas before promoting the adaptive policy."
    else:
        payload["interpretation"] = "F60 is still in progress."

    out = ROOT / args.output_dir
    out.mkdir(parents=True, exist_ok=True)
    json_path = out / "final_stageF60_parking_adaptive_geom_pareto_policy_7000.json"
    json_path.write_text(json.dumps(payload, indent=2), encoding="utf-8")

    lines = [
        "# Final Stage F60 - Parking Adaptive Geometry Pareto CSEF Policy 7000",
        "",
        f"Decision: `{payload['decision']}`.",
        "",
        "F60 replaces the fixed ratio sweep with an Adaptive Geometry Pareto CSEF Edit Policy that chooses candidate ratio, candidate cap, risk-sensitive ranking weights, and gate scaling from scene evidence.",
        "",
    ...
    for label, d in (
        ("F60 - no_gate", payload["deltas_f60_minus_no_gate"]),
        ("F60 - strict", payload["deltas_f60_minus_strict"]),
        ("F60 - calibrated", payload["deltas_f60_minus_calibrated"]),
        ("F60 - F58", payload["deltas_f60_minus_f58"]),
        ("F60 - F56", payload["deltas_f60_minus_f56"]),
    ):
    ...
    print(json.dumps({"decision": payload["decision"], "f60_ready": f60["results_ready"], "interpretation": payload["interpretation"]}, indent=2))
