...
DOC = ROOT / "docs/car_model/final_stageF61_parking_adaptive_geom_pareto_cover_policy_7000_report.md"
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--f61_root", default="outputs/carnet/meshprior/stageF61_parking_adaptive_geom_pareto_cover_policy_7000/parking_7000iter_adaptive_geom_pareto_cover_policy")
    ...
    parser.add_argument("--output_dir", default="outputs/carnet/meshprior/stageF61_parking_adaptive_geom_pareto_cover_policy_7000/summary")
...
def main() -> None:
    ...
    rows = [
        ...
        helper._summarize("adaptive_geom_pareto_cover_policy_F61", Path(args.f61_root), args.iteration, True),
        helper._summarize("no_gate_ratio004_7000", Path(args.nogate_root), args.iteration, False),
    ]
    refs = {row["label"]: row for row in rows}
    f61 = refs["adaptive_geom_pareto_cover_policy_F61"]
    ...
    required = [f61, f58, f56, nogate, strict, calibrated, refs["delayed_ratio002_F53"], refs["delayed_ratio001_F54"]]
    payload = {
        "decision": "F61_COMPLETE" if all(row["results_ready"] for row in required) else "F61_IN_PROGRESS",
        "purpose": "adaptive CSEF policy evaluation against fixed-ratio parking rows and fair no-gate baselines",
        "rows": rows,
        "deltas_f61_minus_no_gate": _deltas(f61, nogate),
        "deltas_f61_minus_strict": _deltas(f61, strict),
        "deltas_f61_minus_calibrated": _deltas(f61, calibrated),
        "deltas_f61_minus_f58": _deltas(f61, f58),
        "deltas_f61_minus_f56": _deltas(f61, f56),
    }
    if payload["decision"] == "F61_COMPLETE" and _all_metric_win(payload["deltas_f61_minus_no_gate"]):
        if _all_metric_win(payload["deltas_f61_minus_f56"]):
            payload["interpretation"] = "F61 adaptive CSEF policy strictly beats both no-gate and the strongest fixed-ratio F56 on all six tracked metrics."
        else:
            payload["interpretation"] = "F61 adaptive CSEF policy strictly beats no-gate on all six tracked metrics but remains mixed versus F56."
    elif payload["decision"] == "F61_COMPLETE":
        payload["interpretation"] = "F61 is complete but still mixed versus no-gate; inspect deltas before promoting the adaptive policy."
    else:
        payload["interpretation"] = "F61 is still in progress."

    out = ROOT / args.output_dir
    out.mkdir(parents=True, exist_ok=True)
    json_path = out / "final_stageF61_parking_adaptive_geom_pareto_cover_policy_7000.json"
    ...
    for label, d in (
        ("F61 - no_gate", payload["deltas_f61_minus_no_gate"]),
        ("F61 - strict", payload["deltas_f61_minus_strict"]),
        ("F61 - calibrated", payload["deltas_f61_minus_calibrated"]),
        ("F61 - F58", payload["deltas_f61_minus_f58"]),
        ("F61 - F56", payload["deltas_f61_minus_f56"]),
    ):
    ...
    print(json.dumps({"decision": payload["decision"], "f61_ready": f61["results_ready"], "interpretation": payload["interpretation"]}, indent=2))
