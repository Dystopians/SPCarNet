...
DOC = ROOT / "docs/car_model/final_stageF59_parking_adaptive_pareto_measured_policy_7000_report.md"
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--f59_root", default="outputs/carnet/meshprior/stageF59_parking_adaptive_pareto_measured_policy_7000/parking_7000iter_adaptive_pareto_measured_policy")
    ...
    parser.add_argument("--output_dir", default="outputs/carnet/meshprior/stageF59_parking_adaptive_pareto_measured_policy_7000/summary")
...
def main() -> None:
    ...
    rows = [
        ...
        helper._summarize("adaptive_pareto_measured_policy_F59", Path(args.f59_root), args.iteration, True),
        helper._summarize("no_gate_ratio004_7000", Path(args.nogate_root), args.iteration, False),
    ]
    refs = {row["label"]: row for row in rows}
    f59 = refs["adaptive_pareto_measured_policy_F59"]
    ...
    required = [f59, f58, f56, nogate, strict, calibrated, refs["delayed_ratio002_F53"], refs["delayed_ratio001_F54"]]
    payload = {
        "decision": "F59_COMPLETE" if all(row["results_ready"] for row in required) else "F59_IN_PROGRESS",
        "purpose": "adaptive CSEF policy evaluation against fixed-ratio parking rows and fair no-gate baselines",
        "rows": rows,
        "deltas_f59_minus_no_gate": _deltas(f59, nogate),
        "deltas_f59_minus_strict": _deltas(f59, strict),
        "deltas_f59_minus_calibrated": _deltas(f59, calibrated),
        "deltas_f59_minus_f58": _deltas(f59, f58),
        "deltas_f59_minus_f56": _deltas(f59, f56),
    }
    if payload["decision"] == "F59_COMPLETE" and _all_metric_win(payload["deltas_f59_minus_no_gate"]):
        if _all_metric_win(payload["deltas_f59_minus_f56"]):
            payload["interpretation"] = "F59 adaptive CSEF policy strictly beats both no-gate and the strongest fixed-ratio F56 on all six tracked metrics."
        else:
            payload["interpretation"] = "F59 adaptive CSEF policy strictly beats no-gate on all six tracked metrics but remains mixed versus F56."
    elif payload["decision"] == "F59_COMPLETE":
        payload["interpretation"] = "F59 is complete but still mixed versus no-gate; inspect deltas before promoting the adaptive policy."
    else:
        payload["interpretation"] = "F59 is still in progress."

    out = ROOT / args.output_dir
    out.mkdir(parents=True, exist_ok=True)
    json_path = out / "final_stageF59_parking_adaptive_pareto_measured_policy_7000.json"
    ...
    for label, d in (
        ("F59 - no_gate", payload["deltas_f59_minus_no_gate"]),
        ("F59 - strict", payload["deltas_f59_minus_strict"]),
        ("F59 - calibrated", payload["deltas_f59_minus_calibrated"]),
        ("F59 - F58", payload["deltas_f59_minus_f58"]),
        ("F59 - F56", payload["deltas_f59_minus_f56"]),
    ):
    ...
    print(json.dumps({"decision": payload["decision"], "f59_ready": f59["results_ready"], "interpretation": payload["interpretation"]}, indent=2))
