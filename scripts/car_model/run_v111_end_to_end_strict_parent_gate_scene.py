...
def _default_parent_method(scene: str, iteration: int) -> str:
    return f"ours_{int(iteration)}_v111_train_all_parent_{scene}"


def _default_candidate_method(scene: str, iteration: int) -> str:
    return f"ours_{int(iteration)}_v111_train_even_candidate_{scene}"


def _default_gate_method(scene: str, iteration: int) -> str:
    return f"ours_{int(iteration)}_v111_train_even_odd_parent_gate_{scene}"
...
def build_plan(args: argparse.Namespace) -> dict[str, Any]:
    ...
    parent_field = (
        Path(args.parent_field_path).expanduser().resolve()
        if args.parent_field_path
        else fields_dir / f"{parent_method}_field.pt"
    )
    candidate_field = (
    ...
    parent_build_cmd = _field_build_cmd(
        args,
        model_path=model_path,
        train_bank=train_bank,
        output_field=parent_field,
        split=str(args.parent_field_split),
        view_subset=str(args.parent_field_view_subset),
    )
    parent_render_cmd = _render_cmd(
        args,
        model_path=model_path,
        method_name=parent_method,
        field_path=parent_field,
    )
    candidate_build_cmd = _field_build_cmd(
        args,
        model_path=model_path,
        train_bank=train_bank,
        output_field=candidate_field,
        split=str(args.field_split),
        view_subset=str(args.field_view_subset),
    )
    candidate_render_cmd = _render_cmd(
        args,
        model_path=model_path,
    ...
    gate_cmd = [
        ...
        "--wandb_name",
        str(args.wandb_name or f"v111-end-to-end-strict-{scene}"),
    ]
    if args.calib_lpips:
    ...
    steps = [
        _step("build_train_all_parent_field", parent_build_cmd, logs_dir / "01_build_parent_field.log"),
        _step("render_parent_train_and_test", parent_render_cmd, logs_dir / "02_render_parent.log"),
        _step("build_train_even_candidate_field", candidate_build_cmd, logs_dir / "03_build_candidate_field.log"),
        _step("render_candidate_train_and_test", candidate_render_cmd, logs_dir / "04_render_candidate.log"),
        _step("gate_train_odd_to_test", gate_cmd, logs_dir / "05_parent_gate.log"),
        _step("evaluate_gated_test", eval_cmd, logs_dir / "06_evaluate_gated_test.log"),
    ]
    return {
        "schema_version": 1,
        "method": "v111 end-to-end strict parent-gated orchestration",
        "scene": scene,
        "iteration": iteration,
        ...
        "parent_field_path": str(parent_field),
        ...
        "strict_fairness": {
            "parent_build": {
                "source_bank": "train_v102_bank",
                "split": args.parent_field_split,
                "view_subset": args.parent_field_view_subset,
                "test_gt_usage": "not used to fit parent field",
            },
            "candidate_build": {
                "source_bank": "train_v102_bank",
                "split": args.field_split,
                "view_subset": args.field_view_subset,
                "test_gt_usage": "not used to fit candidate field",
            },
            "gate": {
                "calib_split": args.calib_split,
                "calib_view_subset": args.calib_view_subset,
                "target_split": args.target_split,
                "test_gt_usage": "v109 gate policy fixed on calibration split; target GT copied for final evaluator only",
            },
        },
        "paths": {
            ...
            "orchestration_report_json": str(reports_dir / f"{scene}_v111_end_to_end_strict_parent_gate_report.json"),
            "orchestration_report_md": str(reports_dir / f"{scene}_v111_end_to_end_strict_parent_gate_report.md"),
        },
        "settings": {
...
def _write_markdown(path: Path, report: dict[str, Any]) -> None:
    ...
    lines = [
        f"# v111 end-to-end strict parent gate - {report.get('scene')}",
        "",
        f"- status: `{report.get('status')}`",
        f"- model: `{report.get('model_path')}`",
        f"- train bank: `{report.get('train_v102_bank_path')}`",
        f"- parent field: `{report.get('parent_field_path')}`",
        f"- candidate field: `{report.get('candidate_field_path')}`",
        ...
        f"- strict fairness: `{json.dumps(report.get('strict_fairness', {}), sort_keys=True)}`",
        "",
        "## Metrics",
        "",
        "```json",
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run one-scene v111 end-to-end strict parent-gated orchestration.")
    ...
    parser.add_argument("--output_root", default=f"/dev/shm/peilincai_spcarnet_v111_end_to_end_strict_parent_gate_{_default_run_tag()}")
    ...
    parser.add_argument("--wandb_group", default="v111_end_to_end_strict_parent_gate")
    ...
    parser.add_argument("--parent_field_path", default="")
    ...
    parser.add_argument("--parent_field_split", choices=("train", "test"), default="train")
    parser.add_argument("--parent_field_view_subset", choices=("all", "even", "odd"), default="all")
