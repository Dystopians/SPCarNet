...
DOC = ROOT / "docs/car_model/final_stageF64_parking_adaptive_bulk_fallback_policy_7000_report.md"
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--f62_root", default="outputs/carnet/meshprior/stageF64_parking_adaptive_bulk_fallback_policy_7000/parking_7000iter_adaptive_bulk_fallback_policy")
    ...
    parser.add_argument("--output_dir", default="outputs/carnet/meshprior/stageF64_parking_adaptive_bulk_fallback_policy_7000/summary")
...
def main() -> None:
    ...
    rows = [
        ...
        helper._summarize("adaptive_bulk_fallback_policy_F64", Path(args.f62_root), args.iteration, True),
        helper._summarize("no_gate_ratio004_7000", Path(args.nogate_root), args.iteration, False),
    ]
    refs = {row["label"]: row for row in rows}
    f62 = refs["adaptive_bulk_fallback_policy_F64"]
    ...
    payload = {
        "decision": "F64_COMPLETE" if all(row["results_ready"] for row in required) else "F64_IN_PROGRESS",
        "purpose": "adaptive bulk-fallback CSEF policy evaluation against fixed-ratio parking rows and fair no-gate baselines",
        "rows": rows,
    ...
    if payload["decision"] == "F64_COMPLETE" and _all_metric_win(payload["deltas_f62_minus_no_gate"]):
        if _all_metric_win(payload["deltas_f62_minus_f56"]):
            payload["interpretation"] = "F64 adaptive bulk-fallback CSEF policy strictly beats both no-gate and the strongest fixed-ratio F56 on all six tracked metrics."
        else:
            payload["interpretation"] = "F64 adaptive bulk-fallback CSEF policy strictly beats no-gate on all six tracked metrics but remains mixed versus F56."
    elif payload["decision"] == "F64_COMPLETE":
        payload["interpretation"] = "F64 is complete but still mixed versus no-gate; inspect deltas before promoting the adaptive policy."
    else:
        payload["interpretation"] = "F64 is still in progress."

    out = ROOT / args.output_dir
    out.mkdir(parents=True, exist_ok=True)
    json_path = out / "final_stageF64_parking_adaptive_bulk_fallback_policy_7000.json"
    ...
    for label, d in (
        ("F64 - no_gate", payload["deltas_f62_minus_no_gate"]),
        ("F64 - strict", payload["deltas_f62_minus_strict"]),
        ("F64 - calibrated", payload["deltas_f62_minus_calibrated"]),
        ("F64 - F58", payload["deltas_f62_minus_f58"]),
        ("F64 - F56", payload["deltas_f62_minus_f56"]),
    ):
