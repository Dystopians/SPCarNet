...
DOC = ROOT / "docs/car_model/final_stageF58_parking_adaptive_pareto_policy_7000_report.md"
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--output_dir", default="outputs/carnet/meshprior/stageF58_parking_adaptive_pareto_policy_7000/summary")
...
def main() -> None:
    ...
    required = [f58, f56, nogate, strict, calibrated, refs["delayed_ratio002_F53"], refs["delayed_ratio001_F54"]]
    payload = {
        "decision": "F58_COMPLETE" if all(row["results_ready"] for row in required) else "F58_IN_PROGRESS",
        "purpose": "adaptive CSEF policy evaluation against fixed-ratio parking rows and fair no-gate baselines",
        "rows": rows,
        "deltas_f58_minus_no_gate": _deltas(f58, nogate),
        "deltas_f58_minus_strict": _deltas(f58, strict),
        "deltas_f58_minus_calibrated": _deltas(f58, calibrated),
        "deltas_f58_minus_f56": _deltas(f58, f56),
    }
    if payload["decision"] == "F58_COMPLETE" and _all_metric_win(payload["deltas_f58_minus_no_gate"]):
        if _all_metric_win(payload["deltas_f58_minus_f56"]):
            payload["interpretation"] = "F58 adaptive CSEF policy strictly beats both no-gate and the strongest fixed-ratio F56 on all six tracked metrics."
        else:
            payload["interpretation"] = "F58 adaptive CSEF policy strictly beats no-gate on all six tracked metrics but remains mixed versus F56."
    elif payload["decision"] == "F58_COMPLETE":
        payload["interpretation"] = "F58 is complete but still mixed versus no-gate; inspect deltas before promoting the adaptive policy."
    else:
        payload["interpretation"] = "F58 is still in progress."

    out = ROOT / args.output_dir
    out.mkdir(parents=True, exist_ok=True)
    json_path = out / "final_stageF58_parking_adaptive_pareto_policy_7000.json"
    ...
    for label, d in (
        ("F58 - no_gate", payload["deltas_f58_minus_no_gate"]),
        ("F58 - strict", payload["deltas_f58_minus_strict"]),
        ("F58 - calibrated", payload["deltas_f58_minus_calibrated"]),
        ("F58 - F56", payload["deltas_f58_minus_f56"]),
    ):
    ...
    print(json.dumps({"decision": payload["decision"], "f58_ready": f58["results_ready"], "interpretation": payload["interpretation"]}, indent=2))
