...
DOC = ROOT / "docs/car_model/final_stageF44_bonsai_calibrated_gate_7000_report.md"
...
def _interpret(payload: dict[str, Any]) -> str:
    if payload["decision"] != "F44_COMPLETE":
        return "F44 is incomplete. Wait for render, metrics, geometry, checkpoint, and W&B before judging the calibrated gate."
    cal = payload["calibrated_gate"]
    ...
    commit_ok = cal["committed_rounds"] > strict["committed_rounds"]
    beats_strict_render = d_strict["psnr"] > 0 and d_strict["ssim"] > 0 and d_strict["lpips"] < 0
    matches_nogate_render = d_nogate["psnr"] >= -0.30 and d_nogate["ssim"] >= -0.01 and d_nogate["lpips"] <= 0.01
    parts = []
    if commit_ok:
        parts.append("Calibrated gating fixes the F43 mechanism failure by accepting at least one recoverable candidate round that strict gating rolled back.")
    if beats_strict_render:
        parts.append(
            "It improves over strict gate on all render metrics: "
            f"PSNR {d_strict['psnr']:+.6f}, SSIM {d_strict['ssim']:+.6f}, LPIPS {d_strict['lpips']:+.6f}."
        )
    if matches_nogate_render:
        parts.append(
            "It is close to the no-gate render envelope while preserving counterfactual/validation metadata, "
            "so this is a plausible fix direction rather than a pure removal of safety."
        )
    else:
        parts.append(
            "It still trails no-gate outside the close-match envelope; the next fix should learn scene-aware thresholds "
            "or use probationary commits with post-recovery validation instead of static relaxed thresholds."
        )
    return " ".join(parts)
...
def write_report(
    ...
    payload = {
        "decision": "F44_COMPLETE" if calibrated["results_ready"] and strict["results_ready"] and nogate["results_ready"] else "F44_IN_PROGRESS",
        "purpose": "7000-iteration bonsai calibrated counterfactual gate repair after F43 negative result",
    ...
    json_path = out / "final_stageF44_bonsai_calibrated_gate_7000.json"
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--calibrated_root", required=True)
    parser.add_argument("--strict_root", required=True)
    parser.add_argument("--nogate_root", required=True)
    parser.add_argument("--iteration", type=int, default=7000)
    parser.add_argument(
        "--output_dir",
        default="outputs/carnet/meshprior/stageF44_bonsai_calibrated_gate_7000/summary",
    )
...
def main() -> None:
    args = parse_args()
    helper = _load_helper()
    calibrated = helper._summarize("calibrated_gate_bonsai_ratio002_7000", Path(args.calibrated_root), args.iteration, True)
    strict = helper._summarize("strict_gate_bonsai_ratio002_7000", Path(args.strict_root), args.iteration, True)
