#!/usr/bin/env bash
# CarNet_v0.8 launcher — point-space conditional flow matching.
# Replaces v0.7's residual decoder (which collapsed to a fixed ~0.04
# smoothing filter per the v0.7 diagnosis) with a per-point flow matching
# decoder. Primary training signal is velocity MSE; inference runs 4-step
...
MODEL_CONFIG="${MODEL_CONFIG:-$ROOT/configs/ss3dm_prior/carnet_v0_8/model_carnet_v0_8.yaml}"
TRAIN_CONFIG="${TRAIN_CONFIG:-$ROOT/configs/ss3dm_prior/carnet_v0_8/train_carnet_v0_8.yaml}"
OUTPUT_ROOT="${OUTPUT_ROOT:-$ROOT/outputs/carnet/v0_8}"
RUN_NAME="${RUN_NAME:-carnet_v0_8}"
