...
def run(args: argparse.Namespace) -> dict[str, Any]:
    ...
    for idx, target in enumerate(tqdm(target_frames, desc="apply support-transport calibrator")):
        with torch.no_grad():
            ...
            if idx < int(args.save_example_views):
                ...
                save_image_tensor(selected_image, visual_dir / f"{target.name}_selected_{selected_variant}.png")
    ...
    verdict = (
        f"Selected support-transport output `{selected_variant}` is all-axis safe versus fixed on this target/test split."
        if selected_all_axis_safe_vs_fixed
        else f"Selected support-transport output `{selected_variant}` is not all-axis safe versus fixed on this target/test split."
    )
