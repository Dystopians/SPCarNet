...
def test_dataset_generates_different_online_corruptions_across_repeated_access(tmp_path: Path) -> None:
    ...
    sample_0 = dataset[0]
    sample_1 = dataset[0]
    assert not np.allclose(sample_0["corrupted_points"].numpy(), sample_1["corrupted_points"].numpy())


def test_dataset_can_freeze_corruption_for_eval(tmp_path: Path) -> None:
    patch_dir = tmp_path / "patch_cache" / "Town02" / "Town02__150_streetsurf"
    patch_dir.mkdir(parents=True, exist_ok=True)
    patch_path = patch_dir / "patch_000.npz"
    _write_patch(patch_path, "patch_000")
    ...
    sample_0 = dataset[0]
    sample_1 = dataset[0]
    assert np.allclose(sample_0["corrupted_points"].numpy(), sample_1["corrupted_points"].numpy())
