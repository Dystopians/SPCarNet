...
def _make_random_batch(batch_size: int = 1, num_points: int = 96, num_queries: int = 64) -> dict[str, torch.Tensor]:
...
def test_model_v9_wide_random_forward() -> None:
    batch = _make_random_batch()
    model = LocalPatchDenoiser(model_type="hybrid_v2_wide")
    outputs = model(
    ...
    assert outputs["query_occupancy_logits"].shape == (1, 64)
    assert outputs["intrinsic_difficulty_pred"].shape == (1,)
    assert outputs["quantized_latent"].shape == (1, 512)
    assert outputs["retrieval_embedding"].shape == (1, 256)
    assert outputs["code_indices"].shape == (1,)


def test_model_v9_wide_forward_on_real_v3_patch_batch() -> None:
