# RESULTS/aggregate — GEMS Stage-2/3 evidence pack v4 (E10 aggregation)
...
|---|---|---|
| `all_rows.json` | 234-row corpus dump (195 canonical): method taxonomy, budgets, per-view arrays, cost, g/d scalars, provenance, VOID annotations, duplicate-consistency checks, supervised-job wall-clocks | (input to all) |
...
| `T5_downstream.{md,csv}` + `T5b_r3_trilogy.md` | d1/d2 per scene (incl. D-2 variants, B6R towns) + R3.a/R3.c/R3.b/R3-FINAL verdict tables (script-extracted from summary.json) | E5-DOWN + Stage3 R3-FINAL |
...
| `T7_robustness.md` | Stage3 W1 seed-1 retrain substitute and W2 50% train-view-drop arm; residue waived with explicit limitation language | E7/E8 |
...
**Pack v4 (Stage3 closure, GOAL#C-01/#C-02, 2026-07-04)** folds W1/W2
substitute runs into T7 and R3-FINAL into T5b/NEGATIVE_RESULTS. The final
from-scratch T1 byte-diff verification is PASS in
