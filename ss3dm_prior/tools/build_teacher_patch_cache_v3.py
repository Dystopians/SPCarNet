...
from ss3dm_prior.data.teacher_patch_builder import load_sequence_observed_cache
from ss3dm_prior.data.teacher_patch_builder_v3 import build_teacher_patches_for_sequence_v3
...
def _load_patch_record_v3(patch_file: Path) -> PatchIndexRecord:
    payload = load_patch_npz(patch_file)
    difficulty_components = json.loads(str(payload.get("difficulty_components_json", "{}").item()))
    return PatchIndexRecord(
        ...
        num_visible_clean_points=int(
            payload.get("visible_clean_points", np.zeros((0, 3), dtype=np.float32)).shape[0]  # type: ignore[name-defined]
        ),
        num_hidden_clean_points=int(
            payload.get("hidden_clean_points", np.zeros((0, 3), dtype=np.float32)).shape[0]  # type: ignore[name-defined]
        ),
        visible_support_fraction=float(payload.get("visible_support_fraction", 0.0).item())
        if hasattr(payload.get("visible_support_fraction", 0.0), "item")
        else 0.0,
        hidden_surface_fraction=float(payload.get("hidden_surface_fraction", 0.0).item())
        if hasattr(payload.get("hidden_surface_fraction", 0.0), "item")
        else 0.0,
        free_space_hard_negative_count=int(payload.get("free_space_hard_negative_count", 0).item())
        if hasattr(payload.get("free_space_hard_negative_count", 0), "item")
        else 0,
        difficulty_components_json=difficulty_components,
    )


def _collect_completed_patch_records_v3(out_dir: Path) -> list[PatchIndexRecord]:
    ...
    for stats_path in sorted(out_dir.rglob("sequence_patch_stats.json")):
        ...
        for patch_file in sorted(sequence_dir.glob("*.npz")):
            records.append(_load_patch_record_v3(patch_file))
    return sorted(records, key=lambda record: (record.town_id, record.sequence_id, record.tile_id, record.scale_id))
...
def _process_entry(
    *,
    ...
    if skip_completed_sequences and sequence_stats_path.exists():
        ...
        return {"status": "reused", "sequence_id": sequence_id, "tiles_in": None, "patches_written": existing_count}
    if not observed_cache_path.exists():
        return {"status": "skip_missing_observed_cache", "message": str(observed_cache_path)}
    if not town_cache_dir.exists():
        return {"status": "skip_missing_town_mesh_cache", "message": str(town_cache_dir)}
    ...
    sequence_records = build_teacher_patches_for_sequence_v3(
...
def make_argparser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Build teacher patch cache v3 with multi-scale visible/hidden semantics.")
    ...
    parser.add_argument("--config", required=True, help="Path to the teacher patch v3 YAML file.")
    ...
    parser.add_argument("--out_dir", required=True, help="Output directory for teacher patch cache v3.")
    parser.add_argument("--subsets", nargs="+", default=["train", "val", "test"], choices=["train", "val", "test"], help="Split subsets to process.")
    parser.add_argument("--debug_max_sequences", type=int, default=None, help="Optional sequence cap.")
    parser.add_argument("--debug_max_tiles_per_sequence", type=int, default=None, help="Optional tile cap per sequence.")
    parser.add_argument("--seed", type=int, default=0, help="Sampling seed.")
    parser.add_argument("--num_workers", type=int, default=1, help="Number of parallel worker processes.")
    parser.add_argument("--skip_completed_sequences", action="store_true", help="Reuse completed sequence directories.")
    parser.add_argument("--rebuild_index_only", action="store_true", help="Only rebuild patch_index.jsonl from completed patch files.")
    return parser
...
def main() -> int:
    ...
    entries = select_manifest_entries_by_split(manifest, split_config, subsets=tuple(args.subsets))
    ...
    if not args.rebuild_index_only:
        ...
        if num_workers == 1:
            for entry in entries:
                ...
                print(f"{status_prefix}: {result['sequence_id']} tiles_in={result['tiles_in']} patches_written={result['patches_written']}")
        else:
    ...
    all_records = _collect_completed_patch_records_v3(out_dir)
