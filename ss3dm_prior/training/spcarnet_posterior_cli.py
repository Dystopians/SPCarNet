"""SP-CarNet Stage 3 — posterior encoder training CLI.

Thin wrapper around :class:`PosteriorEncoderTrainer`.
"""
...
from ss3dm_prior.training.spcarnet_posterior import (
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("--decoder_checkpoint", default=None,
                        help="Override the Stage-2 decoder checkpoint path.")
    parser.add_argument("--output_dir", default=None)
    parser.add_argument("--run_name", default=None)
    parser.add_argument("--device", default=None)
    parser.add_argument("--max_steps", type=int, default=0,
                        help="Cap step count for ad-hoc runs (0 = unlimited).")
    args = parser.parse_args(argv)

    model_cfg, loss_cfg, train_cfg = load_configs(args.model_config, args.train_config)
    if args.decoder_checkpoint:
        model_cfg.decoder_checkpoint = args.decoder_checkpoint
    ...
    trainer = PosteriorEncoderTrainer(
        model_cfg=model_cfg,
        loss_cfg=loss_cfg,
        train_cfg=train_cfg,
    )
