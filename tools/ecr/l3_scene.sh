#!/bin/bash
# GEMS Stage-4 GOAL #E-04 (ladder L3): learned per-pixel fusion row for one
...
DST=$G1/ecr_cache/${SCENE}_${TAG}_l3learned

echo "=== L3 $SCENE: hardlink cache clone ==="
mkdir -p $DST
...
echo "=== L3 $SCENE: train fusion net (train-only LOO) ==="
$PY -m tools.ecr.train_fusion --cache $DST --gpu $GPU

echo "=== L3 $SCENE: ECR row + audit ==="
$PY run_eval.py --checkpoint $CKPT --scene $SCENE \
  --out $G1/eval/l3_${SCENE}_${TAG}_learned_v1 --gpu $GPU \
  --renderer ecr --ecr-cache $DST
env CUDA_VISIBLE_DEVICES=$GPU $PY tools/audit_test_path.py \
  --checkpoint $CKPT --scene $SCENE \
  --out $G1/eval/l3_${SCENE}_${TAG}_learned_audit \
