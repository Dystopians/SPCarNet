...
- v1.3.0 (2026-07-11, MINOR — TOPCONF hardening): §4E.1 threat model of the ECR purity audit (defined term "audited"); 4 ADDITIVE scene registrations (tandt_truck, tandt_train, db_drjohnson, db_playroom — the standard 3DGS eval suite; llff8, -r -1, roi None); `hierarchical_mean_ci` (scene-cluster bootstrap) added alongside the stratified form in reporting tools. No existing scene field, metric definition, or renderer behavior changed; banked 1.2.0 rows remain valid.
...
- `run_eval.py --save-renders` (optional, both renderer modes): writes the metric-path output image per test view to `<out>/renders/` — an output artifact for qualitative grids/failure panels; metric code untouched.
