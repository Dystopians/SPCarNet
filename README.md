...
Qualitative/safety addendum: a no-cleanup counter rerun preserves vNext renders and exports a clean-best/base/vNext panel. A later garden audit found that clean/base and vNext target-evidence renders can have different native frame contracts, so the panel tooling now rejects silent resize comparisons and records selected-frame hashes. The same-resolution garden parent-vs-vNext diagnostic shows only a micro-gain (`+0.000139` PSNR / `+0.000003` SSIM / `-0.00000791` LPIPS), and the new effective-margin gate correctly rejects that low-effect candidate to fallback/no-op under strict no-target-GT apply: [counter qualitative panel log](docs/car_model/6-26-vNext-CounterQualitativePanel-RunLog.md), [frame-contract/effective-margin log](docs/car_model/6-26-vNext-FrameContract-and-EffectiveMarginGate-Log.md), [same-resolution garden panel](docs/car_model/vnext_artifacts/accepted_nonzero_qual_panels_20260626/garden_same_resolution_diagnostic/garden_target_parent_vs_vnext_same_resolution.png).
