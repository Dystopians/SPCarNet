...
<div align="center">
  ...
  <a href="docs/car_model/meshsplatting_paper_metric_reconciliation.md">MeshSplatting paper metrics</a> &nbsp;|&nbsp;
...
> **In one sentence (current evidence).** As of 2026-05-07, the corrected **ELA12 fair baseline audit** no longer selects clean Mesh Splatting baselines by train metrics. It selects the coherent clean checkpoint with the best held-out test score (`PSNR + 20 * SSIM - 20 * LPIPS`) and keeps train scores only as diagnostics. Under that corrected contract MeshSplatOpt/SPCarNet is a strict aggregate full-pass on **5 / 5 current method-artifact scenes** (`parking_phone_tiny`, `bonsai`, `courtyard`, `room`, `counter`) across PSNR, SSIM, LPIPS, sparse AbsRel, sparse Depth MAE, sparse normal angle, and triangle count, with 10.25 – 70.00 % triangle reduction. Per-view RGB is **164 / 165** against the selected clean checkpoints and **163 / 165** against the stricter per-view RGB clean envelope, so universal per-view dominance is not claimed. The **OUT2 train-p15 local parent-gated ELA** fixes the parking outdoor mean visual tail but still has one small PSNR tail view versus clean22000; the earlier **F82 fixed adaptive policy v5** remains the no-per-scene-retuning robustness line, and **CTR-SCE certified recovery (SCE21 / SCE24)** remains the no-op-or-improve recovery layer on top.
...
### Cross-scene final package and adaptive-policy line (F1–F82)
...
| **ELA12** | held-out-test clean checkpoint selection, full audit, and qualitative gallery | **`CORRECTED_HELDOUT_TEST_SELECTED_CLEAN_BASELINE_AUDIT_READY`** — 5 / 5 aggregate strict full-pass; 164 / 165 selected-clean per-view RGB pass |
...
| **OUT2** | parking outdoor ELA with train-p15 local parent-consistency gate | **`PARKING_LOCAL_VISUAL_TAIL_REPAIRED_MEAN_WITH_ONE_VIEW_TAIL`** — aggregate wins vs held-out-best clean22000; 53 / 54 per-view RGB full-pass vs clean22000 |
...
- **ELA12 is the corrected fair comparison contract against pure Mesh Splatting.** For each scene, the clean baseline checkpoint is selected from available clean Mesh Splatting candidates by held-out test score, using `PSNR + 20 * SSIM - 20 * LPIPS`. Train scores are reported only as diagnostics because they can prefer longer overfit continuations. Selected baselines are clean9000 for `bonsai`, `courtyard`, `room`, and `counter`, and clean22000 for `parking_phone_tiny`; the method is a strict aggregate full-pass on 5 / 5 scenes. Per-view stress is 164 / 165 against selected clean and 163 / 165 against a metricwise RGB clean envelope.
...
## Corrected Held-Out Clean Baseline Audit (ELA12)

ELA12 is the current paper-facing comparison against the pure Mesh Splatting baseline on the current artifact set. For every scene, the coherent clean checkpoint is selected by held-out test score, `PSNR + 20 * SSIM - 20 * LPIPS`; train scores are retained only as diagnostics. The audit covers the current scene set with complete method artifacts (`parking_phone_tiny`, `bonsai`, `courtyard`, `room`, `counter`). Raw dataset folders without complete method outputs are not included in this headline claim.

<div align="center">
  <img src="assets/ela12_fair_baseline_gallery.png" width="950" alt="ELA12 held-out-test-selected clean baseline versus MeshSplatOpt qualitative montage">
</div>

| scene | held-out-test-selected clean baseline | method | ΔPSNR ↑ | ΔSSIM ↑ | ΔLPIPS ↓ | ΔAbsRel ↓ | ΔDepth MAE ↓ | ΔNormal ° ↓ | tri reduction | full |
|---|---|---|---:|---:|---:|---:|---:|---:|---:|---|
...
| `parking_phone_tiny` | clean22000 | CSEF70 sparse-depth + train-p15 local parent-gated ELA | +0.497 | +0.0267 | -0.0336 | -0.0031 | -0.0144 | -1.0727 | 70.00 % | yes |

Per-view RGB stress test: **164 / 165** held-out views pass PSNR, SSIM, and LPIPS simultaneously against the selected clean checkpoints. The remaining selected-baseline failure is `parking_phone_tiny/00001.png` with dPSNR `-0.0497` while SSIM and LPIPS still improve. A stricter metricwise per-view RGB clean envelope gives **163 / 165**. The qualitative gallery is `outputs/carnet/meshsplatopt/stageELA12_fair_baseline_audit/qualitative_gallery/gallery.html`; the full audit report is [`docs/car_model/stageELA12_fair_baseline_audit_report.md`](docs/car_model/stageELA12_fair_baseline_audit_report.md). W&B audit run: `bn55syns`.
...
OUT2-v6 is the accepted parking render headline for the outdoor visual tail: it keeps F33/F75 topology and sparse-geometry numbers, but uses a train-p15 low-risk frame floor plus a local parent-consistency mask to decide where ELA repair is applied. Against the held-out-best clean22000 baseline it reaches ΔPSNR +0.497, ΔSSIM +0.0267, ΔLPIPS -0.0336 and 53 / 54 per-view RGB full-pass; against clean30000 it still reaches ΔPSNR +0.568, ΔSSIM +0.0298, ΔLPIPS -0.0376 and 54 / 54, but clean30000 is no longer the reviewer-facing headline baseline. Against OUT1-v7 it adds +0.0239 dB, +0.0020 SSIM, and reduces LPIPS by 0.00035. The later F82 fixed global policy v5 remains the accepted multiscene no-per-scene-retuning validation.

### Fixed adaptive-policy validation (F82 v5)

F82 v5 runs one fixed selector policy and one recovery recipe on the four remaining selected scenes. It is intentionally not a per-scene parameter table: the selector adapts from checkpoint evidence, while sparse-depth lambda, LPIPS lambda, topology freeze, and recovery horizon stay fixed. F81/F82 repeat the same policy at two seeds and reach 8 / 8 all-metric wins.
...
### Recipe B — adaptive CSEF policy with tiny LPIPS recovery (F75, compact parking base)
