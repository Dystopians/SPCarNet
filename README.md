...
- [vNext full9 effective-margin gate artifact summary](docs/car_model/vnext_artifacts/full9_effective_margin_gate_20260626_1500/summary/vnext_manifest_summary_enhanced.md)
- [v115 v106-anchored evidence reparent implementation log](docs/car_model/6-26-v115-v106AnchoredEvidenceReparent-Implementation-Log.md)
- [vNext counter clean-best/base/ours qualitative panel log](docs/car_model/6-26-vNext-CounterQualitativePanel-RunLog.md)
- [vNext frame-contract and effective-margin gate log](docs/car_model/6-26-vNext-FrameContract-and-EffectiveMarginGate-Log.md)
- [vNext garden same-resolution diagnostic panel](docs/car_model/vnext_artifacts/accepted_nonzero_qual_panels_20260626/garden_same_resolution_diagnostic/garden_target_parent_vs_vnext_same_resolution.png)
...
- [vNext bicycle rebuild / ready9 full9 input-closure log](docs/car_model/6-26-vNext-BicycleInputRebuild-Ready9-and-Full9InputClosure-Log.md)
...
Short status: `v106 POD-MoE base-preserve` is the current verified quality line and improves over the local clean MeshSplatting baseline on the assembled selected full9 table. `v113b/v113c` are strict-gate safety repairs; they improve safety and partially repair garden v110b, but do not surpass v106. `v114_oof_refit_pod_moe` is currently blocked by artifact/provenance issues: the field exists, but the original detached package is missing `cfg_args`, and substituting another model is correctly rejected by endpoint-hash safety. Latest live addendum: v110 counter failed during field build with return code `-9`, likely due to memory/shared-storage pressure, so the strict branch still needs a lower-memory field-builder rerun.

vNext status: the certified residual surface texture direction is implemented as a realistic research route, but the latest full9 evidence does **not** yet justify promotion as a paper-quality endpoint. The first fixed structure-aware shrink policy completed `9 / 9` strict no-target-GT scenes with `6 / 9` nonzero accepted rows and mean `25.067699 / 0.741260 / 0.306689`, but it remained below clean MeshSplatting and v106. The stricter effective-margin gate full9 rerun is now also complete: `9 / 9` scenes completed, `0 / 9` missing or failed, `9 / 9` protocol audits passed, only `1 / 9` scene accepted nonzero residual output (`counter`), and `8 / 9` scenes used explicit fallback/no-op. Mean full9 metrics are `25.067410` PSNR, `0.741259` SSIM, and `0.306695` LPIPS, with mean changed fraction `0.001371507`. This proves the safety gate can suppress low-effect candidates, but it is still below the local clean MeshSplatting baseline (`25.151682 / 0.749018 / 0.287621`) and below v106 (`25.831280 / 0.760830 / 0.268435`). Treat vNext as an auditable representation/policy milestone and bottleneck diagnosis, not as a quality-superior method yet.

Qualitative/safety addendum: a no-cleanup counter rerun preserves vNext renders and exports a clean-best/base/vNext panel. A later garden audit found that clean/base and vNext target-evidence renders can have different native frame contracts, so the panel tooling now rejects silent resize comparisons and records selected-frame hashes. The same-resolution garden parent-vs-vNext diagnostic shows only a micro-gain (`+0.000139` PSNR / `+0.000003` SSIM / `-0.00000791` LPIPS), and the full9 effective-margin gate confirms that such low-effect rows should fall back rather than be promoted. See [counter qualitative panel log](docs/car_model/6-26-vNext-CounterQualitativePanel-RunLog.md), [frame-contract/effective-margin log](docs/car_model/6-26-vNext-FrameContract-and-EffectiveMarginGate-Log.md), [full9 effective-margin summary](docs/car_model/vnext_artifacts/full9_effective_margin_gate_20260626_1500/summary/vnext_manifest_summary_enhanced.md), and [same-resolution garden panel](docs/car_model/vnext_artifacts/accepted_nonzero_qual_panels_20260626/garden_same_resolution_diagnostic/garden_target_parent_vs_vnext_same_resolution.png).

v115 interface update: the pipeline now includes an explicit evidence-reparent step for v106-anchored residual distillation. `scripts/car_model/ecsr_reparent_surface_evidence_cache.py` rewrites fit/target evidence `rgb_render` and residual fields against a stronger parent render, and `run_vnext_certified_residual_texture_scene.py` exposes `--reparent_fit_parent_render_dir` / `--reparent_target_parent_render_dir`. This closes a real interface gap, but it is not yet a metric win: a fair v115 pilot still requires restored or freshly rendered v106-compatible parent images.
