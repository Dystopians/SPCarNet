...
mentor_report: docs/car_model/6-24-SPCarNet-Mentor-PPT-Complete-Technical-Report-v66.zh.md
```
...
**Current closure status, 2026-06-24:** `NOT COMPLETE`. The strongest auditable endpoint remains Phase-J: under the locally reproduced selected-clean `26000/30000` envelope, same full9 split, and same evaluator, it is `9 / 9` strict on scene-level PSNR/SSIM/LPIPS, `244 / 246` strict on per-view RGB, and removes `7.6479%` triangles on average. The best fixed representation-level policy is v64 fixed auto bin-alpha: it is `9 / 9` non-regressive/tie versus v56 and promotes only `kitchen`, but the mean gain is still `+0.000410080` PSNR, `+0.000000278` SSIM, `-0.000018951` LPIPS. v65 teacher basis and v66 bin-RGB alpha are completed negative diagnostics, not promoted endpoints. Current report: [`Mentor/PPT technical report v66`](docs/car_model/6-24-SPCarNet-Mentor-PPT-Complete-Technical-Report-v66.zh.md); persistent v66 evidence: `outputs/carnet/meshsplatopt/ecsr_phase_v39_multiscene_ssimaware/v66_bin_rgb_alpha_probe_20260624/summary.md`.
...
**v57a reliability-shrink probe, 2026-06-24:** `REAL_PIPELINE_CHANGE_UNDER_PROBE_NOT_PROMOTED`. v57a adds train-policy-val face-alpha reliability shrink to the atlas adapter and runner. The first W&B probe on `counter` remains positive versus v52 (`+0.001548767` PSNR, `+0.000009656` SSIM, `-0.000117481` LPIPS), but is weaker than raw v55d (`-0.001121521` PSNR, `-0.000001907` SSIM, `+0.000059486` LPIPS). This implementation is useful infrastructure but is not promoted as-is; `kitchen` SSIM-risk validation is running. Evidence: [`v57a face-alpha reliability shrink probe`](docs/car_model/6-24-v57a-FaceAlphaReliabilityShrink-Probe-Log.md).
